...
fn check_hash(path: &Path) -> bool {
    ...
    if let Some(x) = disk_file_hashes.get(path) {
        debug!(path=%fmt_path, "file found in cache");
        x.value() == &hash
    } else {
        debug!(path=%fmt_path, "file not found in cache");
...
pub fn get_from_cache(path: &Path) -> Option<String> {
    ...
    debug!(path=%path.to_string_lossy(), "attempting to find {:?} in cache", path);
    RENDERER_CACHE
...
pub fn insert_into_cache(path: PathBuf, html: String) -> bool {
    debug!(path=%path.to_string_lossy(), "inserting newly rendered item into cache");
    RENDERER_CACHE
        .get_or_init(DashMap::new)
        .insert(path, html)
        .map_or(false, |_| true)
